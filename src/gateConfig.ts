...
export const GENERATE_PRICE = Number(import.meta.env.VITE_GENERATE_PRICE ?? "0");

export const GENERATE_CURRENCY = import.meta.env.VITE_GENERATE_CURRENCY ?? "ETH";

// Where the per-generate payment goes (your wallet/treasury). Required if price > 0.
export const PAYMENT_RECIPIENT = (import.meta.env.VITE_PAYMENT_RECIPIENT ?? "") as string;

// Chain the payment runs on (1 = Ethereum mainnet, 8453 = Base, 137 = Polygon…).
export const PAYMENT_CHAIN_ID = Number(import.meta.env.VITE_PAYMENT_CHAIN_ID ?? "1");
